...
export default function App() {
  ...
  const mut = {
    ...
    deleteTests: async (ids) => { if (ids && ids.length) { await supabase.from("tests").delete().in("id", ids); fetchAll(); } },
    bulkUpload: async ({ category, testName, date, parsed }) => {
      ...
      if (toCreate.length) {
        ...
        if (insErr) return { added: 0, created: 0, skipped: [], error: "Naye students banane mein error: " + insErr.message };
...
function BulkDeleteModal({ data, catStudents, onClose, onDelete }) {
  ...
  const confirm = async () => {
    if (!picked) return;
    setBusy(true);
    await onDelete(picked.ids);
    setBusy(false);
    setDone({ name: picked.testName, count: picked.ids.length });
  };
