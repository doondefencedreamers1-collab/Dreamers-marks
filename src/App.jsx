...
const HOUSES = {
  "Major Somnath House":          "#f5b544",
  "Grenadier House":              "#34d399",
  "Wing Commander Jagmohan House":"#60a5fa",
  "Captain Vikram Batra House":   "#fb7185",
  "Major Mukund House":           "#a78bfa",
  "Major Dhan Singh Thapa House": "#2dd4bf",
};
...
const mapHouse = (r) => ({ id: r.id, studentId: r.student_id, house: r.house, date: r.changed_on });

/* Sorted house timeline for one student: [{house, from, to, current}] */
const houseTimeline = (hist, currentHouse) => {
  const sorted = [...(hist || [])].sort((a, b) => new Date(a.date) - new Date(b.date));
  const seq = [];
  sorted.forEach((h) => { if (!h.house) return; if (!seq.length || seq[seq.length - 1].house !== h.house) seq.push({ house: h.house, from: h.date }); });
  if (!seq.length) return currentHouse ? [{ house: currentHouse, from: null, to: null, current: true }] : [];
  return seq.map((s, i) => ({ house: s.house, from: s.from, to: i < seq.length - 1 ? seq[i + 1].from : null, current: i === seq.length - 1 }));
};
...
export default function App() {
  const [data, setData] = useState({ students: [], tests: [], houseHistory: [] });
  const [session, setSession] = useState(null); // {role:'director'} | {role:'student', roll}
  const [loaded, setLoaded] = useState(false);
  const [synced, setSynced] = useState(false);

  const fetchAll = useCallback(async () => {
    // Supabase/PostgREST read ~1000 rows par cap hota hai — saare rows ke liye page-by-page fetch
    const fetchTable = async (table, orderCol) => {
      const pageSize = 1000;
      let from = 0, all = [];
      for (;;) {
        const { data, error } = await supabase.from(table).select("*").order(orderCol).range(from, from + pageSize - 1);
        if (error || !data) break;
        all = all.concat(data);
    ...
    const [st, te, hh] = await Promise.all([
      fetchTable("students", "created_at"),
      fetchTable("tests", "created_at"),
      fetchTable("house_history", "changed_on"),
    ]);
    setData({ students: st.map(mapStudent), tests: te.map(mapTest), houseHistory: hh.map(mapHouse) });
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel("rt-dreamers")
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "house_history" }, fetchAll)
      .subscribe();
  ...
  const mut = {
    addStudent: async (s) => {
      const { data: ins } = await supabase.from("students").insert({ name: s.name, roll: s.roll, category: s.category, house: s.house || null }).select();
      const sid = ins && ins[0] && ins[0].id;
      if (sid && s.house) await supabase.from("house_history").insert({ student_id: sid, house: s.house, changed_on: new Date().toISOString().slice(0, 10) });
      fetchAll();
    },
    updateStudent: async (s) => {
      const old = data.students.find((x) => x.id === s.id);
      await supabase.from("students").update({ name: s.name, roll: s.roll, category: s.category, house: s.house || null }).eq("id", s.id);
      if (s.house && (!old || old.house !== s.house)) await supabase.from("house_history").insert({ student_id: s.id, house: s.house, changed_on: new Date().toISOString().slice(0, 10) });
      fetchAll();
    },
    ...
    deleteTests: async (ids) => {
      ...
      for (const part of chunk(ids, 150)) {
        const { error } = await supabase.from("tests").delete().in("id", part);
        if (error) { await fetchAll(); return { ok: false, deleted, error: error.message }; }
        deleted += part.length;
      }
      await fetchAll();
    ...
    bulkUpload: async ({ category, testName, date, parsed }) => {
      // chunk helper — bade uploads (400+) ke liye Supabase request limits se bachne ke liye
      const chunk = (arr, n) => { const out = []; for (let i = 0; i < arr.length; i += n) out.push(arr.slice(i, i + n)); return out; };
      const rollToId = {};
      const existById = {};
      data.students.forEach((s) => { rollToId[s.roll] = s.id; existById[s.id] = s; });
      ...
      const histRows = [];
      if (toCreate.length) {
        for (const part of chunk(toCreate, 150)) {
          const { data: ins, error: insErr } = await supabase.from("students")
            .insert(part.map((p) => ({ name: p.name, roll: p.roll, category, house: p.house || null }))).select();
          if (insErr) return { added: 0, created, skipped: [], error: "Naye students banane mein error: " + insErr.message };
          (ins || []).forEach((s) => { rollToId[s.roll] = s.id; createdIds.add(s.id); created++; });
        }
        // naye students ka pehla house bhi history me daalo
        toCreate.forEach((p) => { const sid = rollToId[p.roll]; if (sid && p.house) histRows.push({ student_id: sid, house: p.house, changed_on: date }); });
      }
      // backfill house for EXISTING students who have a house in the sheet but not in DB (ya alag hai)
      const seenUpd = new Set();
      for (const p of parsed) {
        const sid = rollToId[p.roll];
        if (!sid || !p.house || createdIds.has(sid) || seenUpd.has(sid)) continue;
        const cur = existById[sid];
        if (cur && cur.house === p.house) continue; // already same, skip
        seenUpd.add(sid);
        const { error: upErr } = await supabase.from("students").update({ house: p.house }).eq("id", sid);
        if (upErr) return { added: 0, created, skipped: [], error: "House update mein error: " + upErr.message };
        histRows.push({ student_id: sid, house: p.house, changed_on: date }); // reshuffle record
      }
      if (histRows.length) {
        const { error: hErr } = await supabase.from("house_history").insert(histRows);
        if (hErr) return { added: 0, created, skipped: [], error: "House history mein error: " + hErr.message };
      }
      const testRows = [];
      parsed.forEach((p) => {
        const sid = rollToId[p.roll];
      ...
      // Re-upload safe: same naam ka purana test (in students ke liye) pehle hata do, phir naya daalo (chunked)
      const sids = testRows.map((r) => r.student_id);
      for (const part of chunk(sids, 150)) {
        const { error: dErr } = await supabase.from("tests").delete().eq("category", category).eq("test_name", testName).in("student_id", part);
        if (dErr) return { added: 0, created, skipped, error: "Purana test hatane mein error: " + dErr.message };
      }
      let added = 0;
      for (const part of chunk(testRows, 150)) {
        const { error: tErr } = await supabase.from("tests").insert(part);
        if (tErr) return { added, created, skipped, error: "Marks insert karne mein error: " + tErr.message };
        added += part.length;
      }
      await fetchAll();
...
function Director({ data, mut, synced, onLogout }) {
  ...
  const [delTestModal, setDelTestModal] = useState(false);
  ...
  return (
    <>
      ...
      <main className="cpt-main">
        {view === "dashboard" ? (
          <Dashboard data={data} catStudents={catStudents} studentTests={studentTests}
            ...
            onBulk={() => setBulkModal(true)} onFindStudent={findByRoll} onDeleteTest={() => setDelTestModal(true)} />
        ) : view === "category" ? (
          ...
          <StudentView student={liveStudent} tests={studentTests(liveStudent.id)} rankOf={rankOf} readOnly={false}
            ...
            houseHistory={data.houseHistory.filter((h) => h.studentId === liveStudent.id)}
      ...
      {delTestModal && (
        <BulkDeleteModal data={data} catStudents={catStudents} onClose={() => setDelTestModal(false)} onDelete={mut.deleteTests} />
      )}
    </>
  );
}
      ...
      <main className="cpt-main">
        {!student ? (
          ...
          <StudentView student={student} tests={tests} rankOf={rankOf} readOnly={true} allTests={data.tests} allStudents={data.students}
            houseHistory={data.houseHistory.filter((h) => h.studentId === student.id)} />
        )}
      </main>
    </>
  );
...
function Dashboard({ data, catStudents, studentTests, onOpenCategory, onUpload, onBulk, onFindStudent, onDeleteTest }) {
    ...
    <div className="fade-in">
      <div className="page-head">
        ...
        <div className="head-actions">
          <button className="btn ghost danger" onClick={onDeleteTest}><Trash2 size={16} /> Delete Test</button>
...
function StudentView({ student, tests, rankOf, readOnly, allTests = [], allStudents = [], houseHistory = [], onBack, onAddTest, onEditTest, onDeleteTest, onEditStudent, onDeleteStudent }) {
  ...
  const houseSegs = houseTimeline(houseHistory, student.house);
    ...
    <div className="fade-in">
      ...
      <div className="student-head" style={{ "--c": c.color }}>
        ...
        <div className="student-head-main"><h1 className="page-title">{student.name}</h1>
          <p className="page-desc"><span className="tag inline" style={{ "--c": c.color }}>{c.label}</span>
            {student.house && <span className="tag inline" style={{ "--c": houseColor(student.house), marginLeft: 6 }}>{student.house}</span>}
      ...
      {tests.length === 0 ? (
        ...
        <>
          ...
          {houseSegs.length > 0 && (
            <>
              <h2 className="section-label"><Home size={15} style={{ verticalAlign: "-2px" }} /> House History</h2>
              <div className="house-hist">
                {[...houseSegs].reverse().map((seg, i) => (
                  <div className="hh-row" key={i} style={{ "--c": houseColor(seg.house) }}>
                    <span className="hh-dot" />
                    <div className="hh-main">
                      <div className="hh-name">{seg.house}{seg.current && <span className="hh-now">Current</span>}</div>
                      <div className="hh-period">
                        {seg.from ? fmtDate(seg.from) : "Shuru se"} {seg.current ? "– ab tak" : seg.to ? "– " + fmtDate(seg.to) : ""}
                      </div>
                    </div>
                  </div>
                ))}
...
function BulkUploadModal({ catStudents, existingTests = [], onClose, onConfirm }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      if (!rows.length) return setErr("Sheet khaali hai. Template ka format use karein.");

      // Subject columns SHEET ke headers se nikaalo (Roll/Name/House ke alawa).
      // "Maths (/300)" -> { name:"Maths", key:"Maths (/300)", maxMarks:300 }
      const headerKeys = Object.keys(rows[0]);
      const isMeta = (k) => { const lk = k.toLowerCase(); return lk.includes("roll") || lk.includes("name") || lk.includes("house"); };
      const sheetSubs = headerKeys.filter((k) => !isMeta(k)).map((k) => {
        const mm = k.match(/\(\s*\/?\s*(\d+(?:\.\d+)?)\s*\)/);  // bracket ke andar ka number
        const cname = k.replace(/\(.*?\)/g, "").trim();         // (...) hata do
        const def = cleanSubs.find((s) => cname.toLowerCase().includes(s.name.toLowerCase()) || s.name.toLowerCase().includes(cname.toLowerCase()));
        const maxMarks = mm ? Number(mm[1]) : (def ? Number(def.maxMarks) : 100);
        return { name: cname || k, key: k, maxMarks };
      }).filter((s) => s.maxMarks > 0);
      if (!sheetSubs.length) return setErr("Koi subject column nahi mila. Header me 'Subject (/max)' likhein, jaise 'Maths (/300)'.");
      ...
      rows.forEach((row) => {
        ...
        if (!roll) return;
        const subs = sheetSubs.map((s) => {
          let m = Number(row[s.key]) || 0;
          if (m < 0) m = 0;
        ...
        {summary ? (
          summary.error ? (
            <div className="bulk-summary">
              ...
              <h3>Upload fail hua</h3>
              <p className="skip-note" style={{ maxWidth: 420 }}><AlertCircle size={14} /> {summary.error}</p>
              <p style={{ fontSize: 13, opacity: 0.7 }}>
                {summary.added ? `${summary.added} tests insert ho chuke the, phir error aaya.` : "Koi test insert nahi hua — DB ne reject kiya."}
              </p>
              <button className="btn primary full" onClick={() => setSummary(null)}>Wapas jaayein</button>
            </div>
          ) : (
          <div className="bulk-summary">
...
/* ───────────────────────── Bulk Delete (whole-class test) ───────────────────────── */

function BulkDeleteModal({ data, catStudents, onClose, onDelete }) {
  const [cat, setCat] = useState(null);
  const [pickedKey, setPickedKey] = useState(null);
  const [busy, setBusy] = useState(false);
  const [done, setDone] = useState(null);

  const catTests = cat ? data.tests.filter((t) => t.category === cat) : [];
  const map = {};
  catTests.forEach((t) => {
    const key = (t.testName || "").trim().toLowerCase() + "|" + (t.date || "");
    if (!map[key]) map[key] = { key, testName: t.testName, date: t.date, ids: [], pcts: [] };
    map[key].ids.push(t.id); map[key].pcts.push(t.percentage);
  });
  const groups = Object.values(map).sort((a, b) => new Date(b.date) - new Date(a.date));
  const picked = groups.find((g) => g.key === pickedKey) || null;

  const confirm = async () => {
    if (!picked) return;
    setBusy(true);
    const res = await onDelete(picked.ids);
    setBusy(false);
    if (res && res.error) setDone({ error: res.error, count: res.deleted || 0 });
    else setDone({ name: picked.testName, count: (res && res.deleted) || picked.ids.length });
  };

  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal lg" onClick={(e) => e.stopPropagation()}>
        <div className="modal-head"><h3 className="modal-title"><Trash2 size={18} /> Delete a Test (whole class)</h3><button className="x" onClick={onClose}><X size={18} /></button></div>

        {done ? (
          done.error ? (
            <div className="bulk-summary">
              <AlertTriangle size={40} color="#fb7185" />
              <h3>Delete fail hua</h3>
              <p className="skip-note" style={{ maxWidth: 420 }}><AlertCircle size={14} /> {done.error}</p>
              <p style={{ fontSize: 13, opacity: 0.7 }}>{done.count ? `${done.count} delete ho chuke the, phir error aaya.` : "Kuch delete nahi hua — DB ne reject kiya (zyada chance RLS delete policy)."}</p>
              <button className="btn primary full" onClick={() => setDone(null)}>Wapas jaayein</button>
            </div>
          ) : (
          <div className="bulk-summary">
            <CheckCircle2 size={40} color="#34d399" />
            <h3>Deleted!</h3>
            <p><b>{done.name}</b> ke <b>{done.count}</b> students ke marks delete ho gaye.</p>
            <button className="btn primary full" onClick={onClose}>Done</button>
          </div>
          )
        ) : (
          <>
            ...
            <div className="class-pick">
              {CAT_ORDER.map((k) => { const c = CATEGORIES[k]; return (
                <button key={k} className={"class-btn" + (cat === k ? " on" : "")} style={{ "--c": c.color }} onClick={() => { setCat(k); setPickedKey(null); }}>
                  <c.icon size={18} /> {c.label}<small>{catStudents(k).length} students</small>
                </button>); })}
            </div>

            {cat && (<>
              <p className="step-label">2 · Test chuniye (jise delete karna hai)</p>
              {groups.length === 0 ? (
                <div className="empty soft">Is class me koi test nahi mila.</div>
              ) : (
                <div className="del-list">
                  {groups.map((g) => (
                    <button key={g.key} className={"del-row" + (pickedKey === g.key ? " on" : "")} onClick={() => setPickedKey(g.key)}>
                      <div className="del-main"><div className="del-name">{g.testName}</div>
                        <div className="del-sub">{fmtDate(g.date)} · {g.ids.length} students</div></div>
                      {pickedKey === g.key && <CheckCircle2 size={18} color="#fb7185" />}
                    </button>
                  ))}
                </div>
              )}
            ...
            {picked && (
              <div className="auth-err" style={{ marginTop: 14, background: "rgba(251,113,133,.1)" }}>
                <AlertTriangle size={14} /> <b style={{ margin: "0 4px" }}>{picked.testName}</b> ke <b style={{ margin: "0 4px" }}>{picked.ids.length}</b> students ke marks permanently delete honge. Wapas nahi aayenge.
              </div>
            )}

            <div className="modal-foot">
              <button className="btn ghost" onClick={onClose}>Cancel</button>
              <button className="btn danger-solid" disabled={!picked || busy} onClick={confirm}>{busy ? "Deleting…" : picked ? `Delete ${picked.ids.length} marks` : "Delete"}</button>
            </div>
          </>
        )}
      </div>
...
.house-hist{display:flex;flex-direction:column;gap:0;margin-bottom:18px;}
.hh-row{display:flex;gap:12px;position:relative;padding:0 0 14px 4px;}
.hh-row:not(:last-child)::before{content:"";position:absolute;left:9px;top:16px;bottom:-2px;width:2px;background:var(--line2);}
.hh-dot{width:14px;height:14px;border-radius:50%;background:var(--c);flex-shrink:0;margin-top:2px;box-shadow:0 0 0 3px color-mix(in srgb,var(--c) 22%,transparent);z-index:1;}
.hh-main{flex:1;min-width:0;}
.hh-name{font-weight:700;font-size:14px;display:flex;align-items:center;gap:8px;}
.hh-now{font-size:10px;font-weight:700;padding:2px 7px;border-radius:5px;color:#0c111d;background:#34d399;}
.hh-period{font-size:12px;color:var(--muted);margin-top:1px;}
...
.del-list{display:flex;flex-direction:column;gap:8px;max-height:320px;overflow-y:auto;}
.del-row{display:flex;align-items:center;gap:12px;text-align:left;background:var(--bg2);border:1px solid var(--line2);border-radius:11px;padding:12px 15px;color:var(--tx);cursor:pointer;transition:.15s;}
.del-row:hover{border-color:rgba(251,113,133,.45);}
.del-row.on{border-color:#fb7185;background:rgba(251,113,133,.1);}
.del-main{flex:1;min-width:0;}
.del-name{font-weight:700;font-size:14px;}
.del-sub{font-size:12px;color:var(--muted);margin-top:2px;}
`;
