  ...
  const mut = {
    ...
    bulkUpload: async ({ category, testName, date, parsed }) => {
      ...
      for (const p of parsed) {
        ...
        seenUpd.add(sid);
        await supabase.from("students").update({ house: p.house }).eq("id", sid);
        histRows.push({ student_id: sid, house: p.house, changed_on: date }); // reshuffle record
      }
      if (histRows.length) await supabase.from("house_history").insert(histRows);
      const testRows = [];
      parsed.forEach((p) => {
        const sid = rollToId[p.roll];
      ...
      // Re-upload safe: same naam ka purana test (in students ke liye) pehle hata do, phir naya daalo
      const sids = testRows.map((r) => r.student_id);
      if (sids.length) {
        await supabase.from("tests").delete().eq("category", category).eq("test_name", testName).in("student_id", sids);
      }
